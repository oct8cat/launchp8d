 ...
 * @type object
 */
Launchpad.client = Launchpad.redis.createClient(redisSettings.port || 6379, redisSettings.host || 'localhost');

/**
 * Subscriber-mode redis client reference.
 * @type object
 */
Launchpad.subClient = Launchpad.redis.createClient(redisSettings.port || 6379, redisSettings.host || 'localhost');
    ...
    Launchpad.getTasks(function(err, tasks) {
        ...
        async.each(tasks, function(task, cb) {
            if (task.value.complete) {
                completed += 1;
                Launchpad.deleteTask(task.key, cb);
            } else {
                rerunned += 1;
